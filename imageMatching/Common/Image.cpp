...
Image::Image()
{
	...
	_roi_left = 0;
	_roi_top = 0;
	_roi_right = _image.cols;
	_roi_bottom = _image.rows;
}
...
Image::Image(std::string path, float pCutTop, float pCutLeft, float pCutRight, float pCutBottom)
{
	...
	_roi_left = difL;
	_roi_right = _image.cols - (difR + difL);
	_roi_top = difT;
	_roi_bottom = _image.rows - (difB + difT);
...
void Image::calcHistogram( int sections )
{
	...
	for(int s = 0; s < sections; s++)
	{
		...
		for( int i = 0; i < (int) chan.size(); i++)
		{
			...
			int channels[] = { 0 };
			//cv::cvtColor(chan.at(i),chan.at(i),CV_BGR2GRAY);
			cv::MatND hist;
