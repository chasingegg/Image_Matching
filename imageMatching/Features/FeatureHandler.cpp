...
void FeatureHandler::findGoodMatches( Image* image1, Image* image2, float ratio )
{
	...
	symmetryTest( m1, m2, image1, image2 );
...
void FeatureHandler::symmetryTest( image_match* im1, image_match* im2, Image* image1, Image* image2 )
{
	...
	Opitimize(image1, image2, goodMatches);
	...
	final_size = int( im1->goodMatches.size() );
}
...
void FeatureHandler::Opitimize( Image* image1, Image* image2, std::vector< cv::DMatch > &good_matches )
{
		std::vector<cv::DMatch> tmpVector;
		for( std::vector< cv::DMatch > ::const_iterator opt_matches = good_matches.begin();
				opt_matches != good_matches.end(); opt_matches++ )
		{
			int Indx1 = opt_matches->queryIdx;
			int Indx2 = opt_matches->trainIdx;

			int x1 = image1->_keypoints[Indx1].pt.x;
			int y1 = image1->_keypoints[Indx1].pt.y;
			int x2 = image2->_keypoints[Indx2].pt.x;
			int y2 = image2->_keypoints[Indx2].pt.y;

			/*bool atLeft1 = y1 < image1->_image.cols/3;
			bool atTop1 = x1 < image1->_image.rows/3;
			bool atRight1 = y1 > image1->_image.cols*2/3;
			bool atBottom1 = x1 > image1->_image.rows*2/3;

			bool atLeft2 = y2 < image2->_image.cols/3;
			bool atTop2 = x2 < image2->_image.rows/3;
			bool atRight2 = y2 > image2->_image.cols*2/3;
			bool atBottom2 = x2 > image2->_image.rows*2/3;*/

			bool atLeft1 = y1 < image1->_roi_left + (image1->_roi_right - image1->_roi_left)/2;
			bool atTop1 = x1 < image1->_roi_top + (image1->_roi_bottom - image1->_roi_top )/2;
			bool atRight1 = y1 > image1->_roi_right - (image1->_roi_right - image1->_roi_left)/2;
			bool atBottom1 = x1 > image1->_roi_bottom - (image1->_roi_bottom - image1->_roi_top )/2;

			bool atLeft2 = y2 < image2->_roi_left + (image2->_roi_right - image2->_roi_left)/2;
			bool atTop2 = x2 < image2->_roi_top + (image2->_roi_bottom - image2->_roi_top )/2;
			bool atRight2 = y2 > image2->_roi_right - (image2->_roi_right - image2->_roi_left)/2;
			bool atBottom2 = x2 > image2->_roi_bottom - (image2->_roi_bottom - image2->_roi_top )/2; 

		/*	bool badOnes = ((atLeft1 && atTop1) && ((atLeft2 && atBottom2) || (atRight2)))
						|| ((atLeft1 && atBottom1) && ((atLeft2 && atTop2) || (atRight2)))
						|| ((atRight1 && atTop1) && ((atRight2 && atBottom2) || (atLeft2)))
						|| ((atRight1 && atBottom1) && ((atRight2 && atTop2) || (atLeft2))); */
			bool badOnes = (atLeft1 && atRight2) || (atRight1 && atLeft2) || (atTop1 && atBottom2) || (atBottom1 && atTop2);
			if (!badOnes)
			{
				tmpVector.push_back( cv::DMatch(
						(*opt_matches).queryIdx, (*opt_matches).trainIdx, (*opt_matches).distance) );
			}
		}
		//Log::notice("Sizeaaaaaa "+ Log::to_string(int(tmpVector.size())) );
		good_matches.clear();
		good_matches = tmpVector;
}
